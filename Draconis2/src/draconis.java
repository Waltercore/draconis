...
public class draconis extends Canvas implements Runnable,KeyListener{
	...
	// used final for window size, because this is not going to change.
	private static final long serialVersionUID = 1L;
	public static final int WIDTH = 1000, HEIGHT = 800; //size of the game window
	...
	// importing classes:
	public static Room room; //creating object of the class Room
	public static Dragon dragon; //creating object of the class Dragon
	public Fireball fireball; //creating object of the class Fireball
	public Fireball2 fireball2; //creating object of the class Fireball
	public Enemy enemy; //creating object of the class Enemy
	public Enemy2 enemy2; //creating object of the class Enemy2
	private Menu menu; //creating object of the class Menu
	public Background background; //creating object of the class background
	public GameOver gameover; //creating object of the class gameover
	public Laser laser; //creating object of the class laser
	public Iceball iceball; //creating object of the class Iceball
	public static Player2 player2; //creating object of the class player2


	public static enum STATE{ // creating the different states in the game
		MENU,
		SINGLEPLAYER,
		MULTIPLAYER,
		GAMEOVER,
	...
	public static STATE State = STATE.MENU; // this state variable will check in which state we are in.
	...
	public synchronized void start() {
		...
		thread.start(); // Start the thread and look for the run method, and this will start our game routine.
		}
	...
	public void run() {
		...
		while(running) {
			long now = System.nanoTime(); // setting a certain amount of time where the game updates, limit it and render graphics. 
	...
	private void render() {
		BufferStrategy bs = getBufferStrategy(); //initializing the bufferstrategy which helps to organize memory on our canvas
		if(bs == null) {
			createBufferStrategy(3); //will implement 3 buffers to load 3 images back, to increase speed over time, enough for cpu usage.
			return;
		...
		//disposing our bufferstrategy if not next time it loops will be null.
		 ...
		 if (State == STATE.MULTIPLAYER) {
				...
				if(this.scoreP1 >= 20 && !enemyKilled) {
					...
					enemy.render(g);
					laser.render(g);
				}else if(this.scoreP2 >=20 && !enemyKilled) {
					showingEnemy = true;
					enemy.render(g);
					laser.render(g);
				}
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode() == KeyEvent.VK_Z) { //creating key event, if user press "Z" the dragon of player 1 goes up, by calling the "isPressed" condition in Drache class.
			dragon.isPressed = true;
			
		}
		if(e.getKeyCode() == KeyEvent.VK_N) {//creating key event, if user press "N" the dragon of player 2 goes up, by calling the "isPressed" condition in Player2 class.
	...
	public void keyReleased(KeyEvent e) {
		...
		if(e.getKeyCode() == KeyEvent.VK_Z) { // if Z is not pressed, isPressed is false and moves to isFalling condition.
			dragon.isPressed = false;
			
		}
		if(e.getKeyCode() == KeyEvent.VK_N) {// if N is not pressed, isPressed is false and moves to isFalling condition.
