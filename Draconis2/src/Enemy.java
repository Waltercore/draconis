...
public class Enemy extends Rectangle{
	...
	public Enemy(int x, int y,LinkedList<Rectangle> tubes) {
		setBounds(700,200, 194, 194); 
		this.tubes = tubes;
		...
		BufferedImage sheet = ImageIO.read(getClass().getClassLoader().getResourceAsStream("Ufo.png")); //loading the spritesheet with dragon image.
		
		
		texture[0] = sheet.getSubimage(0, 0, 300, 194); //cropping the spritesheet to only show the first square.
		}catch(IOException e) {}
	}
	...
	public void update() {
		...
		if(this.currentFireball!=null) {
			if (this.intersects(this.currentFireball) && draconis.showingEnemy) {
				this.healthbar--;
				this.currentFireball = null;
				
			} 
			if(this.healthbar == 0 && draconis.showingEnemy) {
	...
	if(this.currentFireball2!=null) {
		if (this.intersects(this.currentFireball2) && draconis.showingEnemy) {
			this.healthbar--;
