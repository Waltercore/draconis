...
public class Player2 extends Rectangle {
	...
	private boolean isFalling = false; // used for gravity condition. private boolean isAlive = true; // used to know
										// if we still need to show the player or remove it from the game when it dies.
	private int frames = 0;
	private Laser laser;
	private Iceball iceball;
	private float gravity = 0.3f;

	public Player2(int x, int y, LinkedList<Rectangle> tubes) {
		...
		try {// this can fail, so we use try and catch to load the spritesheet.
			sheet = ImageIO.read(getClass().getClassLoader().getResourceAsStream("reddragon.png")); // loading the
																									// spritesheet with
																									// dragon image.
	...
	public void update() {
		...
		if (isPressed) {// if the key Z is pressed, it goes up.
			...
			if (frames > 20)
				frames = 20; // setting a limit for the frames.
		}

		if (isFalling) {
			spd += gravity; // adding gravity when the dragon falls, speed of 4 + gravity.
			if (spd > 8)
				spd = 8; // setting a limit of speed no greater than 8.
		}
		// collision system

		if (this.intersects(this.laser) && draconis.showingEnemy) {
			// if the dragon collides with a tube display to game over screen and restart
			// values.
			draconis.player2killed = true;
		...
		for (int i = 0; i < tubes.size(); i++) {
			if (this.intersects(tubes.get(i))) {
				// if the dragon collides with a tube display game over screen and restart
				// values.
				draconis.player2killed = true;
		...
		if (y >= draconis.HEIGHT) {
			// if the dragon falls off the screen display game over screen and restart
			// values.
			draconis.player2killed = true;
