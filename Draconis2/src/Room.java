...
public class Room {
	...
	public Room(int time){
		...
		sheet = ImageIO.read(getClass().getClassLoader().getResourceAsStream("woodtubes.png")); //loading the spritesheet with wood image.
		
		
		texture[0] = sheet.getSubimage(0, 0, WIDTH_TUBES, HEIGHT_TUBES); //cropping the spritesheet to only show the first square.
		}catch(IOException e) {}
	}
	...
	public void update() {
		...
		if(currentTime == time) { // if the current time is equal to the target time, we create a new tube.
			...
			//creation of multiple rectangles to complete a column, each rectangle with 50 px height, to use a collision system and destroy each rectangle when colliding with a fireball.
	...
	public void render(Graphics g) {
		...
		for (int i = 0; i < tubes.size(); i++) {
			...
			g.drawImage(texture[0],rect.x,rect.y,WIDTH_TUBES,HEIGHT_TUBES,null); //rendering the subimage containing the dragon.
